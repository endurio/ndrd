...
// list of commands that we recognize, but for which ndrd has no support because
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []chainjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = chainutil.Amount(v.Value).ToBTC()
...
func createOdrRawResult(chainParams *chaincfg.Params, modr *wire.MsgOdr,
	...
	blkHeight int32, chainHeight int32) (*chainjson.OdrRawResult, error) {
	...
	odrReply := &chainjson.OdrRawResult{
		Hex:      modrHex,
		Odrid:    odrHash,
		Hash:     modr.WitnessHash().String(),
		Size:     int32(modr.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(chainutil.NewOdr(modr).Tx)),
...
func handleEstimateFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.EstimateFeeCmd)
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo.SoftForks = []*chainjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &chainjson.Bip9SoftForkDescription{
...
//
// NOTE: This is a endurio extension originally ported from
...
func handleGetRawMembook(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetRawMembookCmd)
	mb := s.cfg.OdrMemBook
...
func handleGetOrderBook(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetOrderBookCmd)
	mb := s.cfg.OdrMemBook
	var depth float64
...
func handleGetRawOrder(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.GetRawOrderCmd)
	...
	if err != nil {
		return nil, &chainjson.RPCError{
			Code: chainjson.ErrRPCNoOdrInfo,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &chainjson.GetTxOutResult{
		...
		Value:         chainutil.Amount(value).ToBTC(),
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &chainjson.PrevOut{
				...
				Value:     chainutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func handleSendRawOrder(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*chainjson.SendRawOrderCmd)
	...
	// Use 0 for the tag to represent local node.
	order := chainutil.NewOdr(msgOrder)
...
//
// NOTE: This is a endurio extension ported from github.com/decred/dcrd.
...
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	...
	return chainjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="ndrd RPC"`)
