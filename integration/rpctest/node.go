// Copyright (c) 2016 The endurio developers
...
// nodeConfig contains all the args, and data required to launch a ndrd process
...
// arguments returns an array of arguments that be used to launch the ndrd
// process.
...
// command returns the exec.Cmd which will be used to start the ndrd process.
...
// to the ndrd process that is launched via Start().
...
// as the base for the log and data directories for ndrd.
...
// start creates a new ndrd process, and writes its pid in a file reserved for
...
// stop interrupts the running ndrd process process, and waits until it exits
...
// shutdown terminates the running ndrd process, and cleans up all
