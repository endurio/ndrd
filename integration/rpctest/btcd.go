...
var (
	...
	// string until ndrd is compiled. This should not be accessed directly;
	// instead use the function btcdExecutablePath().
	executablePath string
)

// btcdExecutablePath returns a path to the ndrd executable to be used by
// rpctests. To ensure the code tests against the most up-to-date version of
// ndrd, this method compiles ndrd the first time it is called. After that, the
...
func btcdExecutablePath() (string, error) {
	...
	// If ndrd has already been compiled, just use that.
	...
	// Build ndrd and output an executable in a static temp path.
	outputPath := filepath.Join(testDir, "ndrd")
	if runtime.GOOS == "windows" {
		outputPath += ".exe"
	}
	cmd := exec.Command(
		"go", "build", "-o", outputPath, "github.com/endurio/ndrd",
	)
	err = cmd.Run()
	if err != nil {
		return "", fmt.Errorf("Failed to build ndrd: %v", err)
	}
