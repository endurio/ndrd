...
func testSendOutputs(r *Harness, t *testing.T) {
	...
	// input.
	txid := genSpend(chainutil.Amount(5 * chainutil.AtomPerCoin))
	...
	txid = genSpend(chainutil.Amount(500 * chainutil.AtomPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(chainutil.AtomPerCoin, pkScript)
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := chainutil.Amount(250 * chainutil.AtomPerCoin)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := chainutil.Amount(50 * chainutil.AtomPerCoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := chainutil.Amount(numMatureOutputs * 50 * chainutil.AtomPerCoin)
