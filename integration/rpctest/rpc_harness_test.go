...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt types.Amount) *chainhash.Hash {
		...
		// Next, send amt Coin to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(types.Amount(5 * types.AtomPerCoin))
	...
	txid = genSpend(types.Amount(500 * types.AtomPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(types.AtomPerCoin, pkScript)
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 Coin.
	expectedBalance := types.Amount(250 * types.AtomPerCoin)
	...
	// chain.
	expectedBalance = types.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := types.Amount(50 * types.AtomPerCoin)
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 Coin) of mature unspendable
	// outputs.
	expectedBalance := types.Amount(numMatureOutputs * 50 * types.AtomPerCoin)
