...
func CalcBlockSubsidy(height int32, chainParams *chaincfg.Params) *types.Balance {
	if chainParams.SubsidyReductionInterval == 0 {
		return types.NewBalance(baseSubsidy, 0)
	}

	// Equivalent to: baseSubsidy / 2^(height/subsidyHalvingInterval)
	a0 := uint64(baseSubsidy) >> uint(height/chainParams.SubsidyReductionInterval)
	return types.NewBalance(types.Amount(a0), 0)
}
...
func checkBlockHeaderSanity(header *wire.BlockHeader, chainParams *chaincfg.Params, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	if flags&BFNoPoWCheck != BFNoPoWCheck {
		...
		pubKeyHash := chainutil.Hash160(serializedPK)
...
func checkBlockSanity(block *chainutil.Block, chainParams *chaincfg.Params, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *chainutil.Block, chainParams *chaincfg.Params, timeSource MedianTimeSource) error {
...
func CheckTransactionInputs(tx *chainutil.Tx, txHeight int32, utxoView *UtxoViewpoint,
	chainParams *chaincfg.Params) (*types.Balance, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if utxo.IsCoinBase() {
			...
			if blocksSincePrev < coinbaseMaturity {
				str := fmt.Sprintf("tried to spend coinbase "+
					...
					coinbaseMaturity)
				return nil, ruleError(ErrImmatureSpend, str)
			}
		}
		...
		atomInBefore := balance.Amount(token)
		balance.SubValue(utxo.Value())
		if balance.Amount(token) > atomInBefore ||
			abs(balance.Amount(token)) > types.MaxAtom {
			str := fmt.Sprintf("total value of all transaction "+
				"inputs for %v is %v which is higher than max "+
				"allowed value of %v", token, abs(balance.Amount(token)),
				types.MaxAtom)
			return nil, ruleError(ErrBadTxOutValue, str)
		}
	}
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *chainutil.Block,
...
func (b *BlockChain) CheckConnectBlockTemplate(block *chainutil.Block, chainParams *chaincfg.Params) error {
	b.chainLock.Lock()
...
func abs(a types.Amount) types.Amount {
	if a < 0 {
		return -a
	}
	return a
