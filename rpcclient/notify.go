...
type NotificationHandlers struct {
	...
	// result of a rescan request, due to how ndrd may send various rescan
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnBtcdConnected
	case chainjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
// parseBtcdConnectedNtfnParams parses out the connection status of ndrd
...
func (c *Client) NotifyNewOrdersAsync(verbose bool) FutureNotifyNewOrdersResult {
	...
	cmd := chainjson.NewNotifyNewOrdersCmd(&verbose)
	return c.sendCmd(cmd)
}
