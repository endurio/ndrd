...
func (ob *OdrBook) removeOrder(order *btcutil.Odr) {
	txHash := order.Hash()
...
//
// This function is safe for concurrent access.
func (ob *OdrBook) RemoveDoubleSpends(tx *btcutil.Tx) {
	// Protect concurrent access.
	ob.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (ob *OdrBook) checkBookDoubleSpend(order *btcutil.Odr) error {
	for _, txIn := range order.TxIn {
...
func (ob *OdrBook) CheckSpend(op wire.OutPoint) *btcutil.Tx {
	ob.mtx.RLock()
	txR := ob.outpoints[op]
	ob.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (ob *OdrBook) fetchInputUtxos(order *btcutil.Odr) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (ob *OdrBook) FetchOrder(txHash *chainhash.Hash) (*btcutil.Odr, error) {
	// Protect concurrent access.
	ob.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (ob *OdrBook) maybeAcceptOrder(order *btcutil.Odr) (*OdrDesc, error) {
	txHash := order.Hash()
...
//
// This function is safe for concurrent access.
func (ob *OdrBook) ProcessOrder(order *btcutil.Odr) (*OdrDesc, error) {
