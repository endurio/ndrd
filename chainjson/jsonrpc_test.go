...
func TestMarshalResponse(t *testing.T) {
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := chainjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := chainjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	_, err = chainjson.MarshalResponse(make(chan int), nil, nil)
	...
	// can't be marshalled.
	_, err = chainjson.MarshalResponse(1, make(chan int), nil)
