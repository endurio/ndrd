...
func ExampleMarshalCmd() {
	...
	gbCmd := chainjson.NewGetBlockCmd(blockHash, chainjson.Bool(false), nil)
	...
	id := 1
	marshalledBytes, err := chainjson.MarshalCmd(id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := chainjson.MarshalResponse(1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
