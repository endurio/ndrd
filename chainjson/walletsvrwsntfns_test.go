...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "btcdconnected",
			newNtfn: func() (interface{}, error) {
				return chainjson.NewCmd("btcdconnected", true)
			},
			staticNtfn: func() interface{} {
				return chainjson.NewBtcdConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"btcdconnected","params":[true],"id":null}`,
			unmarshalled: &chainjson.BtcdConnectedNtfn{
				Connected: true,
			},
		},
		{
	...
	for i, test := range tests {
		...
		marshalled, err := chainjson.MarshalCmd(nil, test.staticNtfn())
		if err != nil {
		...
		// notifications.
		marshalled, err = chainjson.MarshalCmd(nil, cmd)
		if err != nil {
			t.Errorf("MarshalCmd #%d (%s) unexpected error: %v", i,
				test.name, err)
			continue
