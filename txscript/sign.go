...
// WitnessSignature creates an input witness stack for tx to spend Coin sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt types.Amount,
	...
	pk := (*chainec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
// SignatureScript creates an input signature script for tx to spend Coin sent
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *chainec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := chainec.ParseDERSignature(tSig, chainec.S256())
		if err != nil {
			continue
		}
...
type KeyDB interface {
	GetKey(chainutil.Address) (*chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(chainutil.Address) (*chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address chainutil.Address) (*chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
