...
func genRandomSig() (*chainhash.Hash, *chainec.Signature, *chainec.PublicKey, error) {
	privKey, err := chainec.NewPrivateKey(chainec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := chainec.ParseSignature(sig1.Serialize(), chainec.S256())
	key1Copy, _ := chainec.ParsePubKey(key1.SerializeCompressed(), chainec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := chainec.ParseSignature(sig.Serialize(), chainec.S256())
		keyCopy, _ := chainec.ParsePubKey(key.SerializeCompressed(), chainec.S256())
	...
	sigNewCopy, _ := chainec.ParseSignature(sigNew.Serialize(), chainec.S256())
	keyNewCopy, _ := chainec.ParsePubKey(keyNew.SerializeCompressed(), chainec.S256())
