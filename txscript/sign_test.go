...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr chainutil.Address) (*chainec.PrivateKey,
			bool, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*chainec.PublicKey)(&key.PublicKey).
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := chainec.NewPrivateKey(chainec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*chainec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			...
			key2, err := chainec.NewPrivateKey(chainec.S256())
			if err != nil {
			...
			pk2 := (*chainec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := chainec.PrivKeyFromBytes(chainec.S256(), privKeyD)
