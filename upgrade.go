// Copyright (c) 2013-2014 The endurio developers
...
// oldBtcdHomeDir returns the OS specific home directory ndrd used prior to
// version 0.3.3.  This has since been replaced with chainutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "ndrd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".ndrd")
	}
...
// location prior to ndrd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to ndrd
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ndrd.db"), "mainnet")
...
// upgradeDataPaths moves the application data from its location prior to ndrd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		...
		// Move old ndrd.conf into new location if needed.
...
// doUpgrades performs upgrades to ndrd as new versions require it.
