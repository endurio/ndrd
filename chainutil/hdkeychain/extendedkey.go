...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := chainec.S256().ScalarBaseMult(k.key)
		pubKey := chainec.PublicKey{Curve: chainec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(chainec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilNum.Mod(ilNum, chainec.S256().N)
		...
		ilx, ily := chainec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := chainec.ParsePubKey(k.key, chainec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := chainec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := chainec.PublicKey{Curve: chainec.S256(), X: childX, Y: childY}
...
// ECPubKey converts the extended key to a chainec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*chainec.PublicKey, error) {
	return chainec.ParsePubKey(k.pubKeyBytes(), chainec.S256())
}

// ECPrivKey converts the extended key to a chainec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*chainec.PrivateKey, error) {
	...
	privKey, _ := chainec.PrivKeyFromBytes(chainec.S256(), k.key)
	return privKey, nil
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(chainec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(chainec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := chainec.ParsePubKey(keyData, chainec.S256())
		if err != nil {
			return nil, err
		}
	}
