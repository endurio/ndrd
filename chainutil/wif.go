...
func NewWIF(privKey *chainec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + chainec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + chainec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+chainec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+chainec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+chainec.PrivKeyBytesLen]
	privKey, _ := chainec.PrivKeyFromBytes(chainec.S256(), privKeyBytes)
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + chainec.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
	...
	a = paddedAppend(chainec.PrivKeyBytesLen, a, w.PrivKey.D.Bytes())
	if w.CompressPubKey {
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*chainec.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
